...
export class BambuImplementation {
  ...
  async print3mf(
    ...
    const validateTrayValue = (v: unknown, label: string): void => {
      if (!Number.isInteger(v) || (v as number) < -1 || ((v as number) > 15 && (v as number) < 128) || (v as number) > 254) {
        throw new Error(
    ...
    if (isH2) {
      ...
      amsMapping2 = amsMapping.map((v) => {
        ...
        if (v >= 128) return { ams_id: 128, slot_id: v - 128 }; // AMS-HT
        if (v === 254) return { ams_id: 254, slot_id: 254 };    // external
